...
/*   Updated: 2025/01/24 11:58:16 by rguigneb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	exec_command(command_t *command)
{
	...
	if (fork_id == 0)
	{
		...
		{
			command_name = ft_strjoin(get_full_path(path_env[i++]), command->argv[0]);
			execve(command_name, (char *const *)command->argv, (char *const *)command->envp);
		}
		free_garbadge();
		exit(EXIT_FAILURE);
	}
